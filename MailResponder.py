...
# --- LLM Interaction (This is the worker function for the thread pool) ---
def _get_llm_reply_task(student_email, full_history_string, problem_info, latest_student_message_cleaned, problem_level_idx_for_prompt):
    global ollama 
    logging.info(f"LLM-tråd startad för {student_email} (Nivå {problem_level_idx_for_prompt+1})")
    if not OLLAMA_MODEL: 
        logging.error(f"LLM-tråd ({student_email}): OLLAMA_MODEL ej satt."); 
        return {"ulla_reply": "Tappade tråden...", "is_solved": False, "error": True}
    ...
    eval_prompt = f"**Utvärdering:** Matchar SENASTE \"Support:\"-meddelande nyckelorden '{problem_info['losning_nyckelord']}'? (Bedöm om andemeningen stämmer, inte exakta ord.)\n**Svarsinstruktion:**\n1. Första raden: `[LÖST]` eller `[EJ_LÖST]`.\n2. Ny rad: Svara som Ulla."
    ...
    try:
        ...
        logging.info(f"LLM-tråd ({student_email}): Marker: '{marker}'. Löst: {is_solved}. Svar: '{ulla_reply[:50]}...'")
        return {"ulla_reply": ulla_reply, "is_solved": is_solved, "error": False}
    except Exception as e:
        logging.error(f"LLM-tråd ({student_email}): Ollama LLM-fel: {e}", exc_info=True)
        return {"ulla_reply": "Tankeverksamheten krånglar...", "is_solved": False, "error": True}
...
def get_evaluator_decision(student_email, problem_description, solution_keywords, latest_student_message_cleaned):
    ...
    if not OLLAMA_MODEL:
        logging.error(f"Evaluator ({student_email}): OLLAMA_MODEL ej satt."); return "[EJ_LÖST]" # Default to not solved on error

    logging.info(f"Evaluator AI för {student_email}: Utvärderar studentens meddelande.")
...
Svara ENDAST med '[LÖST]' eller '[EJ_LÖST]'."""
    ...
    try:
        response = ollama.chat(
            model=OLLAMA_MODEL, # Could be a different, faster model optimized for classification
            messages=messages_for_evaluator,
            options={'temperature': 0.1, 'num_predict': 1000}, # Low temp, short output for classification
            **ollama_client_args
        )
        raw_eval_reply = response['message']['content'].strip()
        # logging.debug(f"--- RÅTT SVAR FRÅN EVALUATOR AI ({student_email}) ---\n{raw_eval_reply}\n--- SLUT ---")

        if raw_eval_reply == "[LÖST]":
            logging.info(f"Evaluator AI ({student_email}): Bedömning [LÖST]")
            return "[LÖST]"
        else: # Default to EJ_LÖST if not an exact match, or if it errors/gives other text
            if raw_eval_reply != "[EJ_LÖST]":
                 logging.warning(f"Evaluator AI ({student_email}): Oväntat svar '{raw_eval_reply}', tolkar som [EJ_LÖST].")
            else:
                logging.info(f"Evaluator AI ({student_email}): Bedömning [EJ_LÖST]")
            return "[EJ_LÖST]"
    except Exception as e:
...
def get_ulla_persona_reply(student_email, full_history_string_for_ulla, problem_info_for_ulla, 
    ...
    global ollama
    if not OLLAMA_MODEL:
        logging.error(f"Ulla Persona ({student_email}): OLLAMA_MODEL ej satt."); return "Glömde vad jag skulle säga..."

    logging.info(f"Ulla Persona AI för {student_email} (Nivå {problem_level_idx_for_ulla+1}): Genererar svar baserat på '{evaluator_decision_marker}'.")
...
"""
    ...
    try:
        response = ollama.chat(
            model=OLLAMA_MODEL, # This might be your main conversational model (e.g., 12B)
            messages=messages_for_ulla,
            options={'temperature': 0.75, 'num_predict': 1000}, # Higher temp for more natural Ulla
            **ollama_client_args
        )
...
def graph_check_emails():
    ...
    for msg_graph_item in unread_messages_raw:
        ...
        if student_next_eligible_level_idx < NUM_LEVELS:
            expected_start_phrase = START_PHRASES[student_next_eligible_level_idx].lower()
            if active_problem_info: 
                if email_data["cleaned_body"].lower().strip().startswith(expected_start_phrase): is_valid_start_command = True
            else: 
                if (email_data["subject"] and expected_start_phrase in email_data["subject"].lower()) or \
                   (email_data["cleaned_body"].lower().strip().startswith(expected_start_phrase)): is_valid_start_command = True
        
        if is_valid_start_command:
            logging.info(f"Main: Giltigt startkommando för nivå {student_next_eligible_level_idx + 1} från {email_data['sender_email']}.")
            emails_to_process_sequentially.append({"type": "start_command", "data": email_data, "level_idx": student_next_eligible_level_idx })
        elif active_hist_str and active_problem_info:
            logging.info(f"Main: E-post från {email_data['sender_email']} är svar på aktivt problem. Köar för LLM-bearbetning.")
            ...
            if not body_for_llm_task:
                logging.warning(f"Main: Tomt meddelande från {email_data['sender_email']}. Markerar som läst.")
                mark_email_as_read(email_data["graph_msg_id"]); processed_ids_in_phase1.add(email_data["graph_msg_id"]); continue
            ...
            llm_tasks_to_submit.append({
                "email_data_for_result": email_data, # Changed key name
            ...
            emails_to_process_sequentially.append({"type": "inform_wrong_level_or_ignore", "data": email_data, "level_idx": student_next_eligible_level_idx })
    ...
    if llm_tasks_to_submit:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor: 
            future_to_task_package = { # Store the whole package
                executor.submit(
                    ...
                    task["active_problem_convo_id_db"], # Pass through
                    task["email_data_for_result"]      # Pass through
                ): task for task in llm_tasks_to_submit
            }
            for future in as_completed(future_to_task_package):
                # original_task_data = future_to_task_package[future] # This was the input package to submit
                try:
                    llm_result_package = future.result() # This is the output package from the worker
                    llm_results_from_threads.append(llm_result_package) 
                except Exception as exc_llm_future:
                    # original_task_data_failed = future_to_task_package[future] # Get data of failed task
                    # logging.error(f"LLM-uppgift för e-post ID {original_task_data_failed['email_data_for_result']['graph_msg_id']} genererade ett undantag: {exc_llm_future}", exc_info=True)
                    # Log the exception. The email will remain unread and be retried.
                    # It's tricky to get the email_id here if future.result() itself failed before returning the package
            ...
            if graph_send_email(email_data["sender_email"], reply_s, final_ulla_reply, 
                ...
                if is_solved:
                    ...
                    update_student_level(email_data["sender_email"], 
                                         result_package["active_problem_level_idx"] + 1, 
             ...
             if result_package.get("mark_as_read", False): # If worker explicitly said to mark read
                 mark_email_as_read(email_data['graph_msg_id'])
                 processed_ids_in_phase1.add(email_data["graph_msg_id"])
    ...
    for task in emails_to_process_sequentially:
        ...
        if task["type"] == "start_command":
            ...
            # ... (logic for informing about wrong level or ignoring) ...
            ...
            if any_start_phrase_detected_seq and attempted_level_idx_seq != student_level_idx_seq :
                logging.warning(f"Main: Student {email_data_seq['sender_email']} använde startfras '{any_start_phrase_detected_seq}' (Nivå {attempted_level_idx_seq + 1}), men aktuell nivå är {student_level_idx_seq + 1}.")
                msg = f"Hej! Du använde startfrasen för Nivå {attempted_level_idx_seq + 1}. "
                msg += f"För Nivå {student_level_idx_seq + 1}, använd: \"{START_PHRASES[student_level_idx_seq]}\"." if student_level_idx_seq < NUM_LEVELS else "Du har klarat alla nivåer!"
                ...
                logging.warning(f"Main: E-post från {email_data_seq['sender_email']} - ignorerar. Markerar läst.")
            mark_email_as_read(email_data_seq["graph_msg_id"])
            processed_ids_in_phase1.add(email_data_seq["graph_msg_id"])
...
if __name__ == "__main__":
    ...
    if RUN_EMAIL_BOT:
        ...
        try:
            ...
            ollama.show(OLLAMA_MODEL, **ollama_client_args) # Check model existence
